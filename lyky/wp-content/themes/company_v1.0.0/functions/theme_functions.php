...
function head_addons(){
	...
	<?php };
	echo '<meta content="'.$shortname.' v.'.wp_get_theme()->Version.'" name="generator"/>';
	$framework_version_data=get_transient( 'junkie_framework_version_data' );
	echo '<meta content="Theme Junkie Framework v.'.$framework_version_data['version'].'" name="generator"/>';
...
/*------------------------End theme's init functions -----------------------------------*/

/*----- Junkie's useful functions -----*/
...
function show_page_menu($customClass = 'nav clearfix', $addUlContainer = true, $addHomeLink = true){
	...
	//dropdown for pages
	$strdepth = '';
	if (get_option($shortname.'_enable_dropdowns_pages') == 'on')
	...
	if ($is_footer){
		$strdepth="depth=1"; $strdepth2 = $strdepth;
	}
